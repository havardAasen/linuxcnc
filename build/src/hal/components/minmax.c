...
#ifdef MODULE_INFO
MODULE_INFO(linuxcnc, "component:minmax:Track the minimum and maximum values of the input to the outputs");
MODULE_INFO(linuxcnc, "pin:in:float:0:in::None:None");
MODULE_INFO(linuxcnc, "pin:reset:bit:0:in:When reset is asserted, 'in' is copied to the outputs:None:None");
...
static int export(char *prefix, long extra_arg) {
    ...
    r = hal_pin_bit_newf(HAL_IN, &(inst->reset_p), comp_id,
        "%s.reset", prefix);
    if(r != 0) return r;
    r = hal_pin_float_newf(HAL_OUT, &(inst->max__p), comp_id,
        "%s.max", prefix);
    if(r != 0) return r;
    r = hal_pin_float_newf(HAL_OUT, &(inst->min__p), comp_id,
        "%s.min", prefix);
    if(r != 0) return r;
...
RTAPI_MP_INT(count, "number of minmax");
char *names = ""; // comma separated names
RTAPI_MP_STRING(names, "names of minmax");
int rtapi_app_main(void) {
    int r = 0;
    int i;
    comp_id = hal_init("minmax");
    ...
    if(count) {
        for(i=0; i<count; i++) {
            char buf[HAL_NAME_LEN + 1];
            rtapi_snprintf(buf, sizeof(buf), "minmax.%d", i);
            r = export(buf, i);
            if(r != 0) break;
       }
    } else {
...
#line 10 "/home/end/projects/linuxcnc/src/hal/components/minmax.comp"
FUNCTION(_) {
