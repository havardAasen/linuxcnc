...
#ifdef MODULE_INFO
MODULE_INFO(linuxcnc, "component:xor2:Two-input XOR (exclusive OR) gate");
MODULE_INFO(linuxcnc, "pin:in0:bit:0:in::None:None");
MODULE_INFO(linuxcnc, "pin:in1:bit:0:in::None:None");
MODULE_INFO(linuxcnc, "pin:out:bit:0:out:\\fBout\\fR is computed from the value of \\fBin0\\fR and \\fBin1\\fR according\nto the following rule:\n.RS\n.TP\n\\fBin0=TRUE in1=FALSE\\fR\n.TQ\n\\fBin0=FALSE in1=TRUE\\fR\n\\fBout=TRUE\\fR\n.TP\nOtherwise,\n\\fBout=FALSE\\fR:None:None");
...
RTAPI_MP_INT(count, "number of xor2");
char *names = ""; // comma separated names
RTAPI_MP_STRING(names, "names of xor2");
int rtapi_app_main(void) {
    int r = 0;
    int i;
    comp_id = hal_init("xor2");
    ...
    if(count) {
        for(i=0; i<count; i++) {
            char buf[HAL_NAME_LEN + 1];
            rtapi_snprintf(buf, sizeof(buf), "xor2.%d", i);
            r = export(buf, i);
            if(r != 0) break;
       }
    } else {
...
#line 27 "/home/end/projects/linuxcnc/src/hal/components/xor2.comp"
FUNCTION(_) {
    if (( in0 && !in1 ) || ( in1 && !in0 )) {
	out = 1;
    } else {
