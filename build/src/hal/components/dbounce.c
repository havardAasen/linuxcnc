...
static int export(char *prefix, long extra_arg) {
    ...
    r = hal_pin_bit_newf(HAL_OUT, &(inst->out_p), comp_id,
        "%s.out", prefix);
    if(r != 0) return r;
    r = hal_pin_u32_newf(HAL_IN, &(inst->delay_p), comp_id,
        "%s.delay", prefix);
    if(r != 0) return r;
    *(inst->delay_p) = 5;
...
RTAPI_MP_INT(count, "number of dbounce");
char *names = ""; // comma separated names
RTAPI_MP_STRING(names, "names of dbounce");
int rtapi_app_main(void) {
    int r = 0;
    int i;
    comp_id = hal_init("dbounce");
    ...
    if(count) {
        for(i=0; i<count; i++) {
            char buf[HAL_NAME_LEN + 1];
            rtapi_snprintf(buf, sizeof(buf), "dbounce.%d", i);
            r = export(buf, i);
            if(r != 0) break;
       }
    } else {
...
#undef delay
#undef delay_ptr
#define delay_ptr (__comp_inst->delay_p)
#define delay (0+*__comp_inst->delay_p)
#undef state
#define state (__comp_inst->state_p)


#line 16 "/home/end/projects/linuxcnc/src/hal/components/dbounce.comp"
FUNCTION(_) {
