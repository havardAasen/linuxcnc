...
#ifdef MODULE_INFO
MODULE_INFO(linuxcnc, "component:mult2:Product of two inputs");
MODULE_INFO(linuxcnc, "pin:in0:float:0:in::None:None");
MODULE_INFO(linuxcnc, "pin:in1:float:0:in::None:None");
MODULE_INFO(linuxcnc, "pin:out:float:0:out:out = in0 * in1:None:None");
...
RTAPI_MP_INT(count, "number of mult2");
char *names = ""; // comma separated names
RTAPI_MP_STRING(names, "names of mult2");
int rtapi_app_main(void) {
    int r = 0;
    int i;
    comp_id = hal_init("mult2");
    ...
    if(count) {
        for(i=0; i<count; i++) {
            char buf[HAL_NAME_LEN + 1];
            rtapi_snprintf(buf, sizeof(buf), "mult2.%d", i);
            r = export(buf, i);
            if(r != 0) break;
       }
    } else {
...
#line 11 "/home/end/projects/linuxcnc/src/hal/components/mult2.comp"
FUNCTION(_) {
    out = in0 * in1;
}
