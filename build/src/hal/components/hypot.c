...
#ifdef MODULE_INFO
MODULE_INFO(linuxcnc, "component:hypot:Three-input hypotenuse (Euclidean distance) calculator");
...
MODULE_INFO(linuxcnc, "pin:out:float:0:out:out = sqrt(in0^2 + in1^2 + in2^2):None:None");
...
RTAPI_MP_INT(count, "number of hypot");
char *names = ""; // comma separated names
RTAPI_MP_STRING(names, "names of hypot");
int rtapi_app_main(void) {
    int r = 0;
    int i;
    comp_id = hal_init("hypot");
    ...
    if(count) {
        for(i=0; i<count; i++) {
            char buf[HAL_NAME_LEN + 1];
            rtapi_snprintf(buf, sizeof(buf), "hypot.%d", i);
            r = export(buf, i);
            if(r != 0) break;
       }
    } else {
...
#line 10 "/home/end/projects/linuxcnc/src/hal/components/hypot.comp"
