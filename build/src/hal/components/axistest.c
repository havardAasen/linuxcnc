...
#ifdef MODULE_INFO
MODULE_INFO(linuxcnc, "component:axistest:Used to allow testing of an axis. Used IN PnCconf.");
MODULE_INFO(linuxcnc, "pin:jog-minus:bit:0:in:Drive TRUE to jog the axis in its negative ('minus') direction.:None:None");
...
RTAPI_MP_INT(count, "number of axistest");
char *names = ""; // comma separated names
RTAPI_MP_STRING(names, "names of axistest");
int rtapi_app_main(void) {
    int r = 0;
    int i;
    comp_id = hal_init("axistest");
    ...
    if(count) {
        for(i=0; i<count; i++) {
            char buf[HAL_NAME_LEN + 1];
            rtapi_snprintf(buf, sizeof(buf), "axistest.%d", i);
            r = export(buf, i);
            if(r != 0) break;
       }
    } else {
...
FUNCTION(update) {
#line 25 "/home/end/projects/linuxcnc/src/hal/components/axistest.comp"
