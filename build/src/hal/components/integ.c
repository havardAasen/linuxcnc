...
#ifdef MODULE_INFO
MODULE_INFO(linuxcnc, "component:integ:Integrator with gain pin and windup limits");
MODULE_INFO(linuxcnc, "pin:in:float:0:in::None:None");
MODULE_INFO(linuxcnc, "pin:gain:float:0:in::1.0:None");
MODULE_INFO(linuxcnc, "pin:out:float:0:out:The discrete integral of 'gain * in' since 'reset' was deasserted:None:None");
MODULE_INFO(linuxcnc, "pin:reset:bit:0:in:When asserted, set out to 0:None:None");
...
RTAPI_MP_INT(count, "number of integ");
char *names = ""; // comma separated names
RTAPI_MP_STRING(names, "names of integ");
int rtapi_app_main(void) {
    int r = 0;
    int i;
    comp_id = hal_init("integ");
    ...
    if(count) {
        for(i=0; i<count; i++) {
            char buf[HAL_NAME_LEN + 1];
            rtapi_snprintf(buf, sizeof(buf), "integ.%d", i);
            r = export(buf, i);
            if(r != 0) break;
       }
    } else {
...
#line 13 "/home/end/projects/linuxcnc/src/hal/components/integ.comp"
FUNCTION(_) {
    if(reset) out = 0;
