...
#ifdef MODULE_INFO
MODULE_INFO(linuxcnc, "component:bitslice:Converts an unsigned-32 input into individual bits");
MODULE_INFO(linuxcnc, "descr:This component creates individual bit-outputs for each bit of an\nunsigned-32 input. The number of bits can be limited by the \"personality\"\nmodparam.\nThe inverse process can be performed by the weighted_sum HAL component.");
MODULE_INFO(linuxcnc, "pin:in:u32:0:in:The input value:None:None");
MODULE_INFO(linuxcnc, "pin:out-##:bit:(32, 'personality'):out::None:None");
MODULE_INFO(linuxcnc, "author:Andy Pugh");
MODULE_INFO(linuxcnc, "license:GPL2+");
...
static int export(char *prefix, long extra_arg, long personality) {
    ...
    r = hal_pin_u32_newf(HAL_IN, &(inst->in_p), comp_id,
        "%s.in", prefix);
    if(r != 0) return r;
    if((personality) > (32)) {
...
RTAPI_MP_INT(count, "number of bitslice");
char *names = ""; // comma separated names
RTAPI_MP_STRING(names, "names of bitslice");
...
RTAPI_MP_ARRAY_INT(personality, 64, "personality of each bitslice");
...
int rtapi_app_main(void) {
    ...
    if(count) {
        for(i=0; i<count; i++) {
            char buf[HAL_NAME_LEN + 1];
            rtapi_snprintf(buf, sizeof(buf), "bitslice.%d", i);
            r = export(buf, i, p_value("bitslice", buf, i) );
            if(r != 0) break;
       }
    } else {
        size_t i, j;
        ...
        for (i = j = idx = 0; i <= length; i++) {
            ...
            if ((c == ',') || (c == '\0')) {
                buf[j] = '\0';
                r = export(buf, idx, p_value("bitslice", buf, idx) );
                if(r != 0) {break;}
                idx++;
                j = 0;
            } else {
...
FUNCTION(_) {
#line 13 "/home/end/projects/linuxcnc/src/hal/components/bitslice.comp"
int i;
