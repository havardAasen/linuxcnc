...
#ifdef MODULE_INFO
MODULE_INFO(linuxcnc, "component:not:Inverter");
...
RTAPI_MP_INT(count, "number of not");
char *names = ""; // comma separated names
RTAPI_MP_STRING(names, "names of not");
int rtapi_app_main(void) {
    int r = 0;
    int i;
    comp_id = hal_init("not");
    ...
    if(count) {
        for(i=0; i<count; i++) {
            char buf[HAL_NAME_LEN + 1];
            rtapi_snprintf(buf, sizeof(buf), "not.%d", i);
            r = export(buf, i);
            if(r != 0) break;
       }
    } else {
...
#line 15 "/home/end/projects/linuxcnc/src/hal/components/not.comp"
FUNCTION(_) { out = ! in; }
