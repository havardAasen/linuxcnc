...
MODULE_INFO(linuxcnc, "pin:maxa:float:0:in::1e20:None");
...
static int export(char *prefix, long extra_arg) {
    ...
    *(inst->max__p) = 1e20;
    ...
    if(r != 0) return r;
    *(inst->maxv_p) = 1e20;
    r = hal_pin_float_newf(HAL_IN, &(inst->maxa_p), comp_id,
        "%s.maxa", prefix);
    if(r != 0) return r;
    *(inst->maxa_p) = 1e20;
    r = hal_pin_u32_newf(HAL_IN, &(inst->smooth_steps_p), comp_id,
        "%s.smooth-steps", prefix);
    if(r != 0) return r;
    *(inst->smooth_steps_p) = 2;
...
RTAPI_MP_INT(count, "number of limit3");
char *names = ""; // comma separated names
RTAPI_MP_STRING(names, "names of limit3");
int rtapi_app_main(void) {
    int r = 0;
    int i;
    comp_id = hal_init("limit3");
    ...
    if(count) {
        for(i=0; i<count; i++) {
            char buf[HAL_NAME_LEN + 1];
            rtapi_snprintf(buf, sizeof(buf), "limit3.%d", i);
            r = export(buf, i);
            if(r != 0) break;
       }
    } else {
...
#undef maxa
#undef maxa_ptr
#define maxa_ptr (__comp_inst->maxa_p)
#define maxa (0+*__comp_inst->maxa_p)
#undef smooth_steps
#undef smooth_steps_ptr
#define smooth_steps_ptr (__comp_inst->smooth_steps_p)
#define smooth_steps (0+*__comp_inst->smooth_steps_p)
#undef in_pos_old
#define in_pos_old (__comp_inst->in_pos_old_p)
#undef out_old
#define out_old (__comp_inst->out_old_p)


#line 26 "/home/end/projects/linuxcnc/src/hal/components/limit3.comp"
