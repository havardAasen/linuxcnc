...
RTAPI_MP_INT(count, "number of invert");
char *names = ""; // comma separated names
RTAPI_MP_STRING(names, "names of invert");
int rtapi_app_main(void) {
    int r = 0;
    int i;
    comp_id = hal_init("invert");
    ...
    if(count) {
        for(i=0; i<count; i++) {
            char buf[HAL_NAME_LEN + 1];
            rtapi_snprintf(buf, sizeof(buf), "invert.%d", i);
            r = export(buf, i);
            if(r != 0) break;
       }
    } else {
...
#line 30 "/home/end/projects/linuxcnc/src/hal/components/invert.comp"
