...
function ErrorCheck () {
    result=$?
    if [ ! -z "$DISPLAY" ]; then
        echo "catch {send -async popimage destroy .}; destroy ." | wish8.6
    fi
...
as well as in the output of the shell command 'dmesg' and in the terminal"
            ...
            wish8.6 $LINUXCNC_TCL_DIR/show_errors.tcl $DEBUG_FILE $PRINT_FILE
        fi
    fi
...
INIVAR=inivar
HALCMD="halcmd $DASHK"
PICKCONFIG="wish8.6 $LINUXCNC_TCL_DIR/pickconfig.tcl"
LINUXCNC_EMCSH=wish8.6
