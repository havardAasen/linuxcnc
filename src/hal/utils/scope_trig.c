...
static void init_list(GtkWidget *list, char *titles[])
{
    ...
    gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(list),
            -1, titles[0], renderer, "text", COL_CHAN, NULL);

    renderer = gtk_cell_renderer_text_new ();
    gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(list),
            -1, titles[1], renderer, "text", COL_SOURCE, NULL);
...
static void add_to_list(GtkWidget *list, char *strs[])
{
    ...
    gtk_list_store_set(store, &iter, COL_CHAN, strs[0], COL_SOURCE, strs[1], -1);
}
