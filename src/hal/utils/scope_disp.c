...
static void alloc_color(GdkRGBA *color, double red, double green, double blue);
...
static int handle_window_expose(GtkWidget * widget, cairo_t *cr, gpointer data);
...
void refresh_display(void)
{
    ...
    {
        cairo_rectangle_int_t rect = {0, 0, disp->width, disp->height};
        cairo_region_t *region = cairo_region_create_rectangle(&rect);
        /* TODO deprecated in GTK 3.22, replaced with gdk_window_begin_draw_frame */
...
static void init_display_window(void)
{
    ...
    g_signal_connect(disp->drawing, "configure-event",
            G_CALLBACK(configure_window), NULL);
    g_signal_connect(disp->drawing, "draw",
	G_CALLBACK(handle_window_expose), NULL);
    g_signal_connect(disp->drawing, "button-release-event",
        G_CALLBACK(handle_release), NULL);
    g_signal_connect(disp->drawing, "button-press-event",
        G_CALLBACK(handle_click), NULL);
    g_signal_connect(disp->drawing, "motion-notify-event",
        G_CALLBACK(handle_motion), NULL);
    g_signal_connect(disp->drawing, "scroll-event",
    ...
    /* allocate colors */
    alloc_color(&disp->color_bg, 0.0, 0.0, 0.0);
    alloc_color(&disp->color_grid, 1.0, 1.0, 1.0);
    alloc_color(&disp->color_baseline, 0.5, 0.5, 0.5);
    for(i = 0; i<16; i++) {
...
static int handle_window_expose(GtkWidget * widget, cairo_t *cr, gpointer data)
{
    ...
    /* FIXME this function got placed because it worked,
       related to the draw event? */
    refresh_display();
        ...
        gpointer data)
{
    ...
    disp->surface = gdk_window_create_similar_surface(gtk_widget_get_window(widget),
